...
class InputForm extends Component {
  ...
  async handleSubmit(event) {
    ...
    try {
      const key = 'b14ad1d5a67199e7697896c18a73a524';
      const today = moment().format('YYYY-MM-DD');
      const { data } = await axios.get(
        `http://api.marketstack.com/v1/eod?access_key=${key}&symbols=${this.state.stock}&date_from=${this.state.date}&date_to=${today}`
      );
